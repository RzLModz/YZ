/*
# RzL - UserBot
# Copyright (C) 2022 RzL-Bot Created By ZeeoneOfc
#
# This file is a part of < https://github.com/zeeone-ofc/RzL/ >
# PLease read the GNU Affero General Public License in
# <https://www.github.com/zeeone-ofc/RzL/blob/v1/LICENSE/>.
*/ 
...
module.exports = RzL = async (RzL, mek) => {
	try {
		...
		const botNumberss = RzL.user.jid + '@c.us'
		const isGroup = from.endsWith('@g.us')
		const sender = mek.key.fromMe ? RzL.user.jid : isGroup ? mek.participant : mek.key.remoteJid
		const ownerNumber = [`${ownernumber}@s.whatsapp.net`] 
		const isOwner = mek.key.fromMe ? RzL.user.jid : ownerNumber.includes(sender)
		const totalchat = await RzL.chats.all()
		const groupMetadata = isGroup ? await RzL.groupMetadata(from) : ''
        ...
        const mentions = (teks, memberr, id) => {
            (id == null || id == undefined || id == false) ? RzL.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : RzL.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
			...
			const checkLimit = (sender) => {
				...
				for (let lmt of _limit) {
					if (lmt.id === sender) {
						...
						if (limitCounts <= 0) return RzL.sendMessage(from,`Limit kamu sudah habis`, text,{ quoted: mek})
						RzL.sendMessage(from, lang.limitcount(isPremium, limitCounts), text, { quoted : mek})
						found = true
					}
				}
					if (found === false) {
			...
			const isLimit = (sender) =>{ 
				...
				for (let i of _limit) {
					if (i.id === sender) {
						...
						if (limits >= limitawal ) {
							position = true
							RzL.sendMessage(from, lang.limitend(pushname), text, {quoted: mek})
							return true
						} else {
							_limit
							position = true
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        RzL.sendMessage(to, media, MessageType.sticker,{quoted:mek})
			...
			const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    RzL.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
            ...
            mime = res.headers['content-type']
            if (mime.split("/")[1] === "gif") {
                return RzL.sendMessage(from, await getBuffer(url), video ,{caption: caption, gifPlayback: true, mentions: men ? men : [], mimetype: 'video/mp4' ,quoted: mek})
                }
            let type = mime.split("/")[0]+"Message"
            if(mime === "application/pdf"){
                return RzL.sendMessage(from, await getBuffer(url), document, {mimetype: 'application/pdf', caption: caption, mentions: men ? men : [], quoted: mek })
            }
            if(mime.split("/")[0] === "image"){
                return RzL.sendMessage(from, await getBuffer(url), image ,{ caption: caption, mentions: men ? men : [], quoted: mek})
            }
            if(mime.split("/")[0] === "video"){
                return RzL.sendMessage(from, await getBuffer(url), video, {caption: caption, mentions: men ? men : [], mimetype: 'video/mp4', quoted: mek})
            }
            if(mime.split("/")[0] === "audio"){
                return RzL.sendMessage(from, await getBuffer(url), audio, {caption: caption, mentions: men ? men : [], mimetype: 'audio/mpeg', quoted: mek })
            }
        }
				...
				const sendButLocation = async (id, text1, desc1, gam1, but = [], options = {}) => {
					them = gam1
					mediaxxaa = await RzL.prepareMessage(id, them, MessageType.location, {thumbnail: them})
				...
				const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
					them = vid1
					mediaxxaa = await RzL.prepareMessage(id, them, MessageType.video)
				...
				const sendButImage = async(id, text1, desc1, vid1, but = [], options = {}) => {
					them = vid1
					mediaxxaa = await RzL.prepareMessage(id, them, MessageType.image, {thumbnail: Buffer.alloc(0)})
		        ...
		        await RzL.sendMessage(from, `Hmm maap nih gua kick, dilarang share link di group ini`, text , {quoted: mek})
		        RzL.groupRemove(from, [kic]).catch((e)=>{reply(`Bot Harus Jadi Admin`)})
		        }
			...
			const isRzL = checkRegisteredUser(sender)
...
switch (command) {
case 'menu': case 'help': case 'RzL':
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
case 'infobot':
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			reply('Update bot selanjutnya silahkan cek YouTube RzLModz melalui link https://youtube.com/channel/UCJi3qEvIX7AZyRowqSTnoig')
break
case 'owner':{
	if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			const vname = RzL.contacts[i] != undefined ? RzL.contacts[i].vname || RzL.contacts[i].notify : undefined
			ini_list.push({
				"displayName": `${ownername}`,
				"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
				})
				}
				hehe = await RzL.sendMessage(from, {
					"displayName": `${ini_list.length} kontak`,
					"contacts": ini_list 
					}, 'contactsArrayMessage', { quoted: mek })
					RzL.sendMessage(from, `Nih Kak Contact Owner Ku, Cuma Sv Nomor Cewe Ya 🤝`, text, {quoted: hehe})
				}
			break
		...
		for(let i of result.medias){
			if(i.url.includes('mp4')){
				let link = await getBuffer(i.url)
                    RzL.sendMessage(from,link,video,{thumbnail: Buffer.alloc(0), quoted: mek,caption: `Instagram •  ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    RzL.sendMessage(from,link,image,{thumbnail: Buffer.alloc(0), quoted: mek,caption: `Instagram • ${i.type}`})                  
                }
            }
...
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					RzL.sendMessage(from,noweem , MessageType.video, {mimetype: 'video/mp4',quoted: mek})
					})
...
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					RzL.sendMessage(from,weem , MessageType.video, {mimetype: 'video/mp4',quoted: mek})
					})
				}).catch((err) => reply(`Link tidak valid`))
...
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					RzL.sendMessage(from,audnha , MessageType.document, {mimetype: 'audio/mpeg',filename: `Tiktok Music.mp3`,quoted: mek})
					})
				}).catch((err) => reply(`Link tidak valid`))
...
case 'spotify':{
    ...
    await RzL.sendMessage(from, thumbnail, image, { quoted: lol, caption: ini_txt })
    get_audio = await getBuffer(get_result.link)
    await RzL.sendMessage(from, get_audio, audio, { mimetype: 'audio/mpeg', filename: `${get_result.title}.mp3`, quoted: mek})
    }
    break
case 'spotifysearch':{
	if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
case 'nhentai':{
 ...
 case 'nhentaipdf':{
    ...
    await RzL.sendMessage(from, ini_buffer, document, { quoted: mek, mimetype: Mimetype.pdf, filename: `${henid}.pdf` })
    }
    break
case 'nhentaisearch':{
	if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
    if (args.length == 0) return reply(`Example: ${prefix + command} RzL`)
    ini_txt = args.join(" ")
    var po = await getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${lolkey}&text=${ini_txt}`)
    RzL.sendMessage(from, po, image, { quoted: mek, caption: "Selesai" })
    }
    break
//maker textpro
...
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
    ...
    RzL.sendMessage(from, gambar, image, { quoted: mek, caption: "Sukses" })
    }
    break
//islami
case 'listsurah':{
...
case 'alquranaudio':{
    ...
    await RzL.sendMessage(from, ini_buffer, audio, { quoted: mek, mimetype: 'audio/mpeg' })
    }
    break
case 'jadwalsholat':{
	if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
case 'daftar': case 'verify': case 'verif':
			if (isRzL) return  reply(lang.regis())
			try {
					ppregis = await RzL.getProfilePicture(sender)
				} catch {
...
case 'memegenerator': case 'memegen':{
									...
									try {
									   ...
									   var mediiia = await RzL.downloadMediaMessage(enmedia)
										...
										RzL.sendMessage(from, resu, image, {caption:'.stikerin bang', thumbnail: Buffer.alloc(0), quoted: mek})
						...
						if (args.length < 1) return reply(`Kirim perintah *${prefix + command}* Alphabot`)
									...
									try {
										...
										var teks2 = args.join(' ')
										var enmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
										var mediia = await RzL.downloadMediaMessage(enmedia)
...
case 'leave':
			...
			setTimeout( () => {
			RzL.sendMessage(from, 'Sayonara👋', text)
			}, 0)
			break
case 'hidetag':
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			RzL.sendMessage(from, options, text)
			break
...
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			RzL.sendMessage(from, yeh, text, { quoted: mek })
			break  
case 'tagall':
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			if (!isGroup) return reply(lang.group())
...
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			if (!isGroupAdmins) return reply(lang.admin(groupName))
			if (!isBotGroupAdmins) return reply(lang.adminB())
					await RzL.groupUpdateSubject(from, `${q}`)
					RzL.sendMessage(from, `Sukses Mengubah Nama Grup Menjadi ${q}`, text, { quoted: mek })
			break          
...
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			if (!isGroupAdmins) return reply(lang.admin(groupName))
			if (!isBotGroupAdmins) return reply(lang.adminB())
					await RzL.groupUpdateDescription(from, `${q}`)
					RzL.sendMessage(from, `Sukses Mengubah Desk Grup Menjadi ${q}`, text, { quoted: mek })
			break   
case 'kick':
if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			if (!isGroup) return reply(lang.group())
...
case 'bc': case 'broadcast':
			...
			var bcnya3 = await RzL.downloadMediaMessage(bcnya2)
					for (let _ of bcnya) {
						RzL.sendMessage(_.jid, bcnya3, image, { caption: `*----「  BROADCAST 」----*\n\n${q}` })
						}
...
case 'nightcore':{
	 if (!isQuotedAudio) return reply('Reply audio nya om')
					...
					exec(`ffmpeg -i ${media} -filter:a atempo=1.06,asetrate=44100*1.25 ${ran}`, (err, stderr, stdout) => {
						...
						RzL.sendMessage(from, hah, audio, {mimetype: 'audio/mpeg', ptt:true, quoted: mek,duration:99999999999999999999999})
						fs.unlinkSync(ran)
					   })}
				  break      
case 'bass': {
...
case 'slowmo': case 'slow':{
								try {
										...
										exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
											...
											RzL.sendMessage(from, uhh, audio, {mimetype: 'audio/mpeg', ptt:true, quoted: mek})
											fs.unlinkSync(ran)
											})
											} catch (e) {
												reply('Error!')
...
case 'robot':{
									...
									exec(`ffmpeg -i ${media} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
										...
										if (err) return reply('Error!')
										hah = fs.readFileSync(ran)
										RzL.sendMessage(from, hah, audio, {mimetype: 'audio/mpeg', ptt: true, quoted: mek})
										fs.unlinkSync(ran)
										})
										}
									break
...
case 'tupai':{
									try {
										...
										exec(`ffmpeg -i ${media} -filter:a "atempo=0.5,asetrate=65100" ${ran}`, (err, stderr, stdout) => {
											...
											if (err) return reply('Error!')
											hah = fs.readFileSync(ran)
											RzL.sendMessage(from, hah, audio, {mimetype: 'audio/mpeg', ptt:true, quoted: mek,duration: 999099})
											fs.unlinkSync(ran)
											})
											 } catch (e) {	
												reply(mess.error)
...
case 'toimg':{
		if (!isQuotedSticker) return reply('Reply stc nya!')
					...
					mediaa = await RzL.downloadAndSaveMediaMessage(encmediaa)
					ran = getRandom('.png')
					exec(`ffmpeg -i ${mediaa} ${ran}`, (err) => {
					fs.unlinkSync(mediaa)
					if (err) return reply('Eror')
					buffer = fs.readFileSync(ran)
					RzL.sendMessage(from, buffer, image, {quoted: mek, thumbnail:Buffer.alloc(0), caption: 'Done'})
					fs.unlinkSync(ran)
					})
					}
					break   
case 'nuliskiri':{
									...
									.on('exit', () => {
										RzL.sendMessage(from, fs.readFileSync('./database/media/nulis/images/buku/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
						...
						case 'nuliskanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										RzL.sendMessage(from, fs.readFileSync('./database/media/nulis/images/buku/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
						...
						case 'foliokiri':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										RzL.sendMessage(from, fs.readFileSync('./database/media/nulis/images/folio/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
						...
						case 'foliokanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										RzL.sendMessage(from, fs.readFileSync('./database/media/nulis/images/folio/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
...
try{
                zee.Facebook(`${q}`).then(async data => {
                    ...
                    RzL.sendMessage(from, ppfb, video, {mimetype:'video/mp4', quoted: mek, caption: txt})
             })} catch {
   ...
   case 'soundcloud':
	if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
				...
				zee.SoundCloud(`${q}`).then(async (data) => {
                    ...
                    RzL.sendMessage(from , await getBuffer(data.medias[0].url), audio,{ quoted: mek, mimetype: 'audio/mpeg' })
				})
			break
	case 'telesticker': case 'tstiker': {
		if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			RzL.sendMessage(from, await getBuffer(telestc[i].url), sticker, {mimetype:'image/webp',quoted: mek})
			}
		}
		break
case 'tebakgambar':
...
fs.writeFileSync("./database/game/tebakgambar.json", JSON.stringify(tebakgambar))
console.log(jawaban)
RzL.sendMessage(from, tebakya, image, {quoted: mek, caption: "\n\nTimeout : 120.00 seconds" })
await sleep(120000)
if (tebakgambar.hasOwnProperty(sender.split('@')[0])) {
RzL.sendMessage(from, "Waktu permainan habis" + '\n\n' +"*Jawaban :*"  + '\n' + '_'+ jawaban +'_', text, {quoted: mek}) // ur cods
delete tebakgambar[sender.split('@')[0]]
fs.writeFileSync("./database/game/tebakgambar.json", JSON.stringify(tebakgambar))
                    }
                    })
...
reply(lang.wait())
	emoji.get(`${qes}`).then(async emojii => {
					...
					console.log(teks)
					//RzL.sendMessage(from, await getBuffer(teks), sticker, {mimetype:'image/webp',quoted: mek})
...
case 'anime':{
	if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
                    ...
                    await RzL.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })}
                    break
case 'manga':{
	if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
                    if (args.length == 0) return reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
...
case 'kusonime':{
	if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
                    ...
                    await RzL.sendMessage(from, ini_buffer, image, { quoted: mek, caption: ini_txt })}
                    break
case 'kusonimesearch':{
	if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
                    if (args.length == 0) return reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
...
case 'cersex2':{
	...
	RzL.sendMessage(from, await getBuffer(txt.result.Thumb), MessageType.image,{quoted: mek, caption: 'Judul : ' + txt.result.Judul + '\n' + 'Cerita : ' + txt.result.Cerita})
	}
	break
...
case 'animequote2':{
	...
	RzL.sendMessage(from, await getBuffer(txt.result.thumb), MessageType.image, {quoted: mek, caption: 'Anime : ' + txt.result.anime + '\n' + 'Karakter : ' + txt.result.character + '\n' + 'Episode : ' + txt.result.episode + '\n' + 'Quotes : ' + txt.result.quotes})
	}
	break
case 'trumpthink':{
	if (!isRzL) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
case "listonline": {
        ...
        try {
          let online = [
            ...Object.keys(RzL.chats.get(id).presences),
            RzL.user.jid,
          ];
          RzL.reply(
            m.chat,
...
case 'nami': case 'copper': case 'naruto':  case 'zoro': case 'ussop': case 'minato': case 'sasuke': case 'sakura':case 'luffy': case 'sanji':  case 'boruto': case 'sarada': case 'mitsuki': case 'orochimaru': case 'tsunade': case 'kakashi': case 'killua': case 'gon': case 'rimuru': case 'sagiri': case 'natsu': case 'tanjirou': case 'nezuko': case 'senku':
            ...
            RzL.sendMessage(from, await getBuffer(txt.result.list), MessageType.image, {caption: 'Wibu 👀', quoted: mek})            
            } catch (e) {
 ...
 try{
                shape = '✓ '
infom = await RzL.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
...
try {
	if (!isOwner) return reply(lang.owner(botname))
return RzL.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`RzLBot :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
